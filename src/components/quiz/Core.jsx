...
function Core({
    ...
    return answers.map((answer, index) => {
      if (answerSelectionType === "single") {
        ...
        answerBtnIncorrectClassName =
          `${userInputIndex}` !== correctAnswer && `${index + 1}` === `${userInputIndex}` ? "incorrect" : "";
        ...
        answerBtnIncorrectClassName =
          !correctAnswer.includes(index + 1) && userInputIndex?.includes(index + 1) ? "incorrect" : "";
  ...
  return (
    <div className="questionWrapper">
      ...
      {timer && !isRunning && (
        <div>
          {appLocale.timerTimeTaken}: <b>{displayTime(timer - timeRemaining)}</b>
        </div>
      )}
        ...
        <div className="questionWrapperBody">
          ...
          {isRunning ? (
            <>
              <h3
                dangerouslySetInnerHTML={rawMarkup(
                  `${activeQuestion && activeQuestion.question} ${appLocale.marksOfQuestion.replace(
                    "<marks>",
              ...
              {activeQuestion &&
                renderTags(answerSelectionTypeState, activeQuestion.correctAnswer.length, activeQuestion.segment)}
