...
function Quiz({
  ...
  const nrOfQuestions =
    quiz.nrOfQuestions && quiz.nrOfQuestions < quiz.questions.length ? quiz.nrOfQuestions : quiz.questions.length;
  ...
  const shuffleAnswerSequence = (oldQuestions = []) => {
    const newQuestions = oldQuestions.map((question) => {
      ...
      const shuffledAnswersWithIndex = answerWithIndex.sort(() => Math.random() - 0.5);
      ...
      if (question.answerSelectionType === "single") {
        ...
        const newCorrectAnswer =
          shuffledAnswersWithIndex.findIndex((ans) => `${ans[1] + 1}` === `${oldCorrectAnswer}`) + 1;
        return {
          ...question,
      ...
      if (question.answerSelectionType === "multiple") {
        ...
        const newCorrectAnswer = oldCorrectAnswer.map(
          (cans) => shuffledAnswersWithIndex.findIndex((ans) => `${ans[1] + 1}` === `${cans}`) + 1
        );
        return {
          ...question,
          correctAnswer: newCorrectAnswer,
  ...
  const validateQuiz = (q) => {
    ...
    if ((timer && typeof timer !== "number") || timer < 1) {
      console.error(
        timer && typeof timer !== "number" ? "timer must be a number" : "timer must be a number greater than 0"
      );
      return false;
    }
